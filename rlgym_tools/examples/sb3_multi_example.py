...
if __name__ == "__main__":  # Required for multiprocessing
    ...
    def get_match():  # Need to use a function so that each instance can call it and produce their own objects
        return Match(
            ...
            self_play=True,
            terminal_conditions=[
                TimeoutCondition(round(fps * 30)),
                GoalScoredCondition(),
            ],  # Some basic terminals
    ...
    env = SB3MultipleInstanceEnv(
        get_match, 2
    )  # Start 2 instances, waiting 60 seconds between each
    ...
    callback = CheckpointCallback(
        round(1_000_000 / env.num_envs), save_path="policy", name_prefix="rl_model"
    )
    ...
    model = PPO.load(
        "policy/rl_model_1000002_steps.zip", env, custom_objects=dict(n_envs=2)
    )
